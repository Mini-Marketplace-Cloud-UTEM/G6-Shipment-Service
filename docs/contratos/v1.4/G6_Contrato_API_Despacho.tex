...
\fancyhead[R]{\small\textcolor{primary}{v1.4}}
...
\begin{tabularx}{\textwidth}{l l X l}
  ...
  \midrule
  \httpmethod{post}{POST}   & \texttt{/shipments/quotes}           & Cotizar envío consolidado        & G4 \\
  \httpmethod{post}{POST}   & \texttt{/shipments}                  & Crear despachos multi-origen     & G5 \\
...
\begin{tabularx}{\textwidth}{l l l c X}
  ...
  \texttt{weight\_kg}          & number   & \texttt{5.0}                  & ---  & Peso facturable equivalente en kg. \\
...
\begin{tikzpicture}[
  ...
  startend/.style={circle, fill=black, inner sep=2pt}
]
  \node[startend]                  (start)     {};
  \node[state, right=1.2cm of start] (pending)   {PENDING};
  ...
  \node[startend, below=0.8cm of cancelled] (end1) {};
  \node[startend, below=0.8cm of failed]    (end2) {};
  \node[startend, below=0.8cm of returned]  (end3) {};
  ...
  \draw[arrow]    (cancelled) -- (end1);
  \draw[arrow]    (failed)    -- (end2);
  \draw[arrow]    (returned)  -- (end3);
\end{tikzpicture}
\end{center}
...
\begin{tabularx}{\textwidth}{l l l X}
  ...
  Entregado   & \texttt{DELIVERED}   & Normal  & $\rightarrow$ \texttt{RETURNED} \\
...
\textbf{Paso 1: Traducción de Talla a Peso Facturable}
Cada paquete enviado en el contrato indica una Talla (\texttt{size}). El motor de cotizaciones de G6 traduce automáticamente esta talla a un Peso Facturable en kilogramos según la siguiente tabla estándar:
\begin{itemize}[noitemsep]
  \item \textbf{XS}: 1.0 kg
  \item \textbf{S}: 2.0 kg
  \item \textbf{M}: 5.0 kg
  \item \textbf{L}: 10.0 kg
  \item \textbf{XL}: 20.0 kg
  \item \textbf{XXL}: 40.0 kg
\end{itemize}
...
\subsection{Cotizar Envíos (Tarifa Consolidada)}

\routeheader{post}{POST}{/api/v1/shipments/quotes}

\textbf{Descripción:} Endpoint inteligente que cotiza envíos en base a la Talla del paquete y la distancia al destino.

\vspace{0.3cm}
\textbf{Cotización por Tallas (Misma, Adyacente o Extrema)}

Requiere que G4 consolide las tallas (G3) y la ciudad de destino (G1).
...
\begin{lstlisting}
{
  ...
  "city": "Santiago",
  "packages": [
    { "originCd": "NORTE", "size": "M" },
    { "originCd": "CENTRO", "size": "XL" }
  ]
}
\end{lstlisting}
...
\begin{tabularx}{\textwidth}{l l l X}
  ...
  \texttt{packages}       & array  & Sí & Lista de paquetes con \texttt{originCd} y \texttt{size}. \\
  \bottomrule
\end{tabularx}
\end{table}
...
\begin{lstlisting}
{
  ...
  "message": "Datos que no pasan validación.",
  "details": [
    {
      "field": "weightKg",
      "message": "El campo weightKg debe ser mayor a 0."
    }
  ],
...
La arquitectura de eventos se basa en un modelo \textbf{Pub/Sub} (Publish/Subscribe) utilizando un \textbf{GCP Pub/Sub} (ej. Upstash GCP Pub/Sub o Supabase Realtime). 
...
% ── SHIPMENT_CREATED ──────────────────────────────────────────────────────────
\subsection{SHIPMENT\_CREATED}

Se emite cuando G5 crea exitosamente un nuevo despacho.

\contractheader{SHIPMENT\_CREATED}
...
% ── SHIPMENT_IN_TRANSIT ────────────────────────────────────────────────────────
\subsection{SHIPMENT\_IN\_TRANSIT}

Se emite cuando el paquete sale a reparto.

\contractheader{SHIPMENT\_IN\_TRANSIT}
...
% ── SHIPMENT_DELIVERED ────────────────────────────────────────────────────────
\subsection{SHIPMENT\_DELIVERED}

Se emite cuando el paquete es entregado exitosamente.

\contractheader{SHIPMENT\_DELIVERED}
...
% ── SHIPMENT_FAILED ───────────────────────────────────────────────────────────
\subsection{SHIPMENT\_FAILED}

Se emite cuando un intento de entrega falla.

\contractheader{SHIPMENT\_FAILED}
...
% ── SHIPMENT_RETURNED ─────────────────────────────────────────────────────────
\subsection{SHIPMENT\_RETURNED}

Se emite cuando un paquete entregado es devuelto.

\contractheader{SHIPMENT\_RETURNED}
...
\begin{enumerate}[noitemsep]
  ...
  \item Publicamos el evento en el tópico \texttt{shipment-events} del GCP Pub/Sub.
  \item G8 consume el evento desde el tópico y notifica al usuario final.
\end{enumerate}

\subsection{JSON que G6 publica en el GCP Pub/Sub (para G8)}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Acuse de recibo (ACK):} Una vez procesada la información
    ...
    \textbf{ACK} (\textit{Acknowledgment}) automático hacia el GCP Pub/Sub para
...
\begin{tabularx}{\textwidth}{c c l l X}
  ...
  G6     & Broker (GCP Pub/Sub/Supabase) & Asíncrono & Publish (Pub)   & Mensaje en tópico \texttt{shipment-events} \\
...
\begin{tikzpicture}[
  ...
  % ── GCP Pub/Sub + consumidores (derecha) ──
  \node[brokerbox, right=2.8cm of g6] (broker) {GCP Pub/Sub\\\relax{\tiny[GCP Pub/Sub / Supabase Realtime]}};
...
CREATE TABLE shipments (
    ...
    city               TEXT NOT NULL,
    originCd          TEXT NOT NULL, -- 'NORTE', 'CENTRO', 'SUR'
...
CREATE TABLE shipment_status_history (
    ...
    shipmentId TEXT REFERENCES shipments(shipmentId),
...
\begin{enumerate}[noitemsep]
  ...
  \item \textbf{Disponibilidad del GCP Pub/Sub:} El GCP Pub/Sub (GCP Pub/Sub o Supabase Realtime) está operativo y accesible. La publicación de eventos es \emph{at-least-once}.
...
\begin{tabularx}{\textwidth}{l X X l}
  ...
  \midrule
  R1 & Caída del GCP Pub/Sub impide que G8 notifique al usuario final. & Cola de reintentos con backoff exponencial; tabla \texttt{outbox} local para eventos pendientes. & Alto \\
...
\begin{tabularx}{\textwidth}{l l l X}
  ...
  1.2 & 20 Jun 2026 & Grupo 6 & Alineación de paginación y esquema de errores unificado (v1.2). \\
  \bottomrule
\end{tabularx}
\end{table}
